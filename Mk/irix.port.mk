...
DEV_ERROR+=	"you cannot include irix.port[.pre].mk twice"
.    endif
...
DEV_ERROR+=	"${PKGNAME}: Makefile error: you cannot include irix.port[.post].mk twice"
	@${FALSE}
.    endif
...
# Build
DO_MAKE_BUILD?=	${SETENV} ${MAKE_ENV} ${MAKE_CMD} ${MAKE_DBG} ${MAKE_FLAGS} ${MAKEFILE} ${_MAKE_JOBS} ${MAKE_ARGS:N${DESTDIRNAME}=*}
.    if !target(do-build)
do-build:
...
do-install:
	@(cd ${INSTALL_WRKSRC} && ${SETENV} ${MAKE_ENV} ${FAKEROOT} ${MAKE_CMD} ${MAKE_DBG} ${MAKE_FLAGS} ${MAKEFILE} ${MAKE_ARGS} ${INSTALL_TARGET})
.    endif

# Test

.    if !target(do-test) && defined(TEST_TARGET)
DO_MAKE_TEST?=	${SETENV} ${TEST_ENV} ${MAKE_CMD} ${MAKE_DBG} ${MAKE_FLAGS} ${MAKEFILE} ${TEST_ARGS:N${DESTDIRNAME}=*}
do-test:
...
makesum: check-sanity
	...
	@${SETENV} \
			...
			dp_ECHO_MSG='${ECHO_MSG}' \
			dp_SCRIPTSDIR='${SCRIPTSDIR}' \
			${SH} ${SCRIPTSDIR}/irix.makesum.sh ${DISTFILES:C/.*/'&'/}
.    endif
...
#	@${SETENV} \
	${SETENV} \
			...
			dp_DISABLE_SIZE='${DISABLE_SIZE}' \
			dp_NO_CHECKSUM='${NO_CHECKSUM}' \
			${SH} ${SCRIPTSDIR}/irix.checksum.sh ${_CKSUMFILES:C/.*/'&'/}
.      endif
.    endif
